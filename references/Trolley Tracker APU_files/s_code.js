...
s.linkInternalFilters="javascript:,apu.edu"
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"

/* Form Analysis Config (should be above doPlugins section) */
s.formList="gs, season_form" /* use form names */
s.trackFormList=false
s.trackPageName=true
s.useCommerce=false
s.varUsed="prop4"
s.eventList="" //Abandon,Success,Error - Only if s.userCommerce=true

// Campaign Stacking
s.campaignStackingExpires=365; //in days
s.campaignStackingCount=10; //number of campaign codes to keep in cache
s.campaignStackingCookieName='s_ct'; //name of cookie that is used
...
function s_doPlugins(s) {
	...
	if(!s.campaign && isNaN(s.getQueryParam('a')) == false) {
	  s.campaign=s.getQueryParam('a');
  }
    
	/* Get the Referring Page Name for Search Term Referer */
	if(!s.prop11) s.prop11=s.getQueryParam('rpn');
  ...
  s.prop3=s.getAndPersistValue(s.prop3,'s_p3_persist',0);

  if(s.prop3) {
      s.prop10=s.prop3 + " : "  + s.pageName;
      s.eVar4=s.prop3;
  }

  s.eVar5=s.campaignStacking();

  s.prop5=s.getNewRepeat();

  s.prop6=s.getTimeParting('h','-5','2007'); // Set hour 
  s.eVar6=s.prop6
  s.prop7=s.getTimeParting('d','-5','2007'); // Set day
  s.eVar7=s.prop7
  s.prop8=s.getTimeParting('w','-5','2007'); // Set Weekend / Weekday
  s.eVar8=s.prop8
  
  s_campaign_temp=s.getAndPersistValue(s.campaign,'s_cp_persist',0);
  
  if(s_campaign_temp) {
      s.prop9=s_campaign_temp + " : " + s.pageName;
  }
  
  // For plugin: getPercentPageViewed v1.2
  /*s.prop13=s.getPreviousValue(s.pageName,"s_pv");
  if (s.prop13){
  	s.prop14=s.getPercentPageViewed();
  }*/
...
/*
 * Plugin: s.campaignStacking : 1.0 - creates a string campaign variables
 * s.campaign variable must be set prior to calling this method else if will
 * return and empty string
 */

s.campaignStacking = new Function(""
+"var s=this;var ay=s.split(s.campaignStackingSuccessEvent,',');for(v"
+"ar u=0;u<ay.length;u++){if(s.events&&s.events.indexOf(ay[u])!=-1){s"
+".c_w(s.campaignStackingCookieName,'');return '';}}if(!s.campaign||s"
+".campaign=='')return '';var arry=new Array();var a=new Array();var "
+"c=s.c_r(s.campaignStackingCookieName);var g=0;var h=new Array();if("
+"c&&c!='') arry=eval(c);var e=new Date();e.setFullYear(e.getFullYear"
+"()+5);if(arry.length>0&&arry[arry.length-1][0]==s.campaign)arry[arr"
+"y.length-1]=[s.campaign, new Date().getTime()];else arry[arry.lengt"
+"h]=[s.campaign, new Date().getTime()];var data=s.join(arry,{delim:'"
+",',front:'[',back:']',wrap:'\\''});var start=arry.length-s.campaign"
+"StackingCount < 0?0:arry.length-s.campaignStackingCount;s.c_w(s.cam"
+"paignStackingCookieName,data,e);for(var x=start;x<arry.length;x++){"
+"var diff=Math.round(new Date()-new Date(parseInt(arry[x][1])))/8640"
+"0000;if(diff<s.campaignStackingExpires){h[g]=arry[x][0];a[g++]=arry"
+"[x];}}var r=s.join(h,{delim:s.campaignStackingDelimiter});return r;"
);
...
s.visitorNamespace="azusapacificuniversity"

/* PLEASE UNCOMMENT THE TWO LINES BELOW AFTER THE CERTIFICATE IS SET UP*/
s.trackingServer="webstats.apu.edu"
s.trackingServerSecure="swebstats.apu.edu"
